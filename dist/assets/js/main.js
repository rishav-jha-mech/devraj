...
const gamedevContainer = document.getElementById("game-dev-projects");
...
function animateProgressF() {
  ...
  progress.forEach((progress) => {
    const _PRG = document.getElementById(progress.id),
      _OUT = document.querySelector(`[for=${progress.id}]`),
      K = 2,
      TMAX = K * _PRG.getAttribute("data-value");
    function load(t = 0) {
      if (t <= TMAX) {
        if (t % K === 0) {
          _OUT.innerHTML = `${t / K}%`;
...
function animateSkillsF() {
  const elements = document
    ...
    .querySelectorAll(".blockHero");
  elements.forEach((obj) => {
    counterAnim(
      obj,
      0,
      parseInt(obj.getAttribute("data-value")),
      obj.getAttribute("data-duration") ?? 1000
    );
  });
}
...
function showProjects(param) {
  switch (param) {
    case "web":
      appBtn.classList.remove("my-active");
      gameBtn.classList.remove("my-active");

      gamedevContainer.classList.add("d-none");
      appdevContainer.classList.add("d-none");

      webBtn.classList.add("my-active");
      webdevContainer.classList.remove("d-none");
      break;
    case "app":
      gameBtn.classList.remove("my-active");
      webBtn.classList.remove("my-active");

      gamedevContainer.classList.add("d-none");
      webdevContainer.classList.add("d-none");

      appBtn.classList.add("my-active");
      appdevContainer.classList.remove("d-none");
      break;
    case "game":
      appBtn.classList.remove("my-active");
      webBtn.classList.remove("my-active");
      ...
      gameBtn.classList.add("my-active");
      gamedevContainer.classList.remove("d-none");
      break;
  }
}
